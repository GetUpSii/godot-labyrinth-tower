...
static var _template_paths = {
	"skills": "res://data/skill/skill_template.json",
...
static func load_static_templates() -> Array:
	...
	for key in _template_paths:
		...
		for type in data:
			var arr = data[type]
			for entry in arr:
				var template: SkillTemplate = SkillTemplate.new()
				_apply_template_data(template, entry)
				templates.append(template)
	return templates

# 从数据字典加载实例
static func load_instance_from_data(data: Dictionary) -> SkillInstance:
	...
	# 通过工厂创建角色
	var instance = SkillFactory.create_skill(template_id)
	if not instance:
...
# 解析模板数据
static func _apply_template_data(template: SkillTemplate ,data: Dictionary) -> void:
	template.set_data(data)
	
# 解析模板数据
static func _apply_instance_data(instance: SkillInstance ,data: Dictionary) -> void:
	# 应用实例数据
