...
static func initialize():
	# 加载所有模板文件
	var templates = CraftLoader.load_static_templates()
...
## 创建合成实例
static func create_craft(template_id: String) -> CraftInstance:
	if not _templates.has(template_id):
		push_error("无法创建合成配方实例: 未知ID - " + template_id)
		return null
		## 尝试动态加载
		#var template = ItemLoader.load_template_by_id(template_id)
			...
			#return null
	var skill: CraftInstance = CraftInstance.new(_templates[template_id])
	return skill
...
# 手动注册模板 (用于动态创建的角色)
static func register_template(_template: CraftTemplate) -> void:
	assert(_template.id != "", "Template must have valid ID")
	_templates[_template.id] = _template

# 获取模板数据
static func get_template(template_id: String) -> CraftTemplate:
